...
\begin{frame}{Word2Vec - Parameters Learning - W' (\(W_2\))}
	\begin{itemize}
		...
		\item \( = \frac{1}{T} \sum_{t=1}^{T} \left( \frac{\partial \left( u_t - \log \left( \sum_{i=1}^{V} exp(u_i) \right)\right)}{\partial u_j} \right)=\)
		\item \( = \frac{1}{T} \sum_{t=1}^{T} \left( \frac{\partial \left( u_t\right)}{\partial u_j} -\frac{\partial \left(\log \left( \sum_{i=1}^{V} exp(u_i) \right)\right)}{\partial u_j} \right)=\)
		\item \( = \frac{1}{T} \sum_{t=1}^{T} \left( t_j - \frac{exp(u_j)}{\sum_{i=1}^{V} exp(u_i)} \right)\)
		\item \( = \frac{1}{T} \sum_{t=1}^{T} \left( t_j - y_j \right) = \), con \(t_j = 1\) se \(j=t\) e \(t_j = 0\) se \(j\neq t\)
		\item \( = \frac{1}{T} \sum_{t=1}^{T} e_j \).
		\item \(\frac{\partial u_j}{\partial w_{ij}'} = \frac{\partial \left( v_{w_j}'^T \cdot h\right)}{\partial w_{ij}'} = h_i\).
		\item  \( \frac{\partial J_{\theta}}{\partial w_{ij}'} = \frac{\partial J_{\theta}}{\partial u_j} \cdot \frac{\partial u_j}{\partial w_{ij}'} = h_i \cdot \frac{1}{T} \sum_{t=1}^{T} e_j\)
	\end{itemize}
\end{frame}
...
\begin{frame}{Word2Vec - Parameters Learning - W' (\(W_2\))}
	...
	\begin{itemize}
		\item \( w_{ij}'^{(t+1)} = w_{ij}'^{(t)} + \eta\cdot h_i \cdot \frac{1}{T} \sum_{t=1}^{T} e_j\), con \(i \in {1,2,..,N}\) e \(j \in {1,2,..,V}\)
		\item \( v_{w_{j}}'^{(t+1)} = v_{w_{j}}'^{(t)} + \eta\cdot h \cdot \frac{1}{T} \sum_{t=1}^{T} e_j\), con \(j \in {1,2,..,V}\)
	\end{itemize}
...
\begin{frame}{Word2Vec - Parameters Learning - W (\(W_1\))}
	\begin{itemize}
		...
		\item \( = \frac{1}{T} \sum_{t=1}^{T} \sum_{j=1}^{V} \left(e_j \cdot \frac{\partial u_j}{\partial h_i}\right) = \)
		\item \( = \frac{1}{T} \sum_{t=1}^{T} \sum_{j=1}^{V} \left(e_j \cdot w_{ij}'\right) = \)
		\item \( = \frac{1}{T} \sum_{t=1}^{T} \sum_{j=1}^{V} \left(e_j \cdot w_{ij}'\right) =\)
		\item \( = \frac{1}{T} \sum_{t=1}^{T} \left(e \cdot W_{2_{i}}^T\right)\), con \(W_{2_{i}}^T\) la i-esima riga di \(W_2\) transposta, in modo tale da avere un vettore \([V,1]\).
		\item \(\frac{\partial h_i}{\partial w_{ki}} = x_k\)
		\item \(\frac{\partial J_{\theta}}{\partial w_{ki}} = \frac{\partial J_{\theta}}{\partial h_i} \cdot \frac{\partial h_i}{\partial w_{ki}} = x_k \cdot \frac{1}{T} \sum_{t=1}^{T} \left(e \cdot W_{2_{i}}^T\right)\)
	\end{itemize}
\end{frame}
